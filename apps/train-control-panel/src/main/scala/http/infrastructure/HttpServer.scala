...
object HttpServer {
  def stream[F[_]: Async: Temporal: Trace](
    ...
    val finalHttpApp = Http4sLogger.httpApp(logHeaders = true, logBody = true)(httpApp(entryPoint))
  ...
  def httpApp[F[_]: Async: Temporal: Trace](entryPoint: EntryPoint[F]): HttpApp[F] =
    (logicRoutes(entryPoint) <+> docsRoute(entryPoint)).orNotFound

  def logicRoutes[F[_]: Async: Trace](entryPoint: EntryPoint[F]): HttpRoutes[F] =
    B3Propagation
      .make[F, Kleisli[F, Span[F], *]](OpenApiEndpoints.routes)
